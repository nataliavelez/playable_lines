...
export class Game extends Scene {
  ...
  handlePlayerStateChange({ playerId, changes }) {
    console.log('🎮 Starting state change for player:', playerId, 'with changes:', changes);
    
    if (!this.gridEngine?.hasCharacter(playerId)) {
        console.warn('❌ GridEngine not ready or missing player:', playerId);
        return;
    }
    ...
    if (changes.position) {
        const currentPos = this.gridEngine.getPosition(playerId);
        console.log('📍 Movement request:', {
            playerId,
            from: currentPos,
            to: changes.position,
            isCurrentPlayer: playerId === this.playerId,
            isMoving: this.gridEngine.isMoving(playerId),
            direction: changes.direction
        });

        if (currentPos.x !== changes.position.x || currentPos.y !== changes.position.y) {
            try {
                if (playerId === this.playerId) {
                    console.log('🚶 Attempting to move current player');
                    this.gridEngine.move(playerId, changes.direction);
                } else {
                    console.log('🎯 Setting position for other player');
                    this.gridEngine.setPosition(playerId, changes.position);
                }
                
                // Log final position after movement
                const finalPos = this.gridEngine.getPosition(playerId);
                console.log('✅ Movement complete:', {
                    playerId,
                    finalPosition: finalPos,
                    succeeded: finalPos.x === changes.position.x && finalPos.y === changes.position.y
                });
                
                this.playMoveAnimation(playerId, changes.direction);
            } catch (error) {
                console.error('💥 Movement failed:', {
                    ...
                    gridEngineState: {
                        hasCharacter: this.gridEngine.hasCharacter(playerId),
                        isMoving: this.gridEngine.isMoving(playerId),
                        currentPos: this.gridEngine.getPosition(playerId)
                    }
                });
            }
        }
  ...
  if (changes.direction && changes.direction !== this.gridEngine.getFacingDirection(playerId)) {
      this.gridEngine.turnTowards(playerId, changes.direction);
      if (!this.gridEngine.isMoving(playerId)) {
          player.sprite.play(`idle_${changes.direction}`);
      }
  }

  // Handle carrying changes
  if (changes.carrying !== undefined && changes.carrying !== player.carrying) {
      player.carrying = changes.carrying;
      player.indicator.visible = changes.carrying;
      ...
      this.playWaterAnimation(playerId, direction);
      ...
      if (changes.carrying) {
          ...
          const position = this.gridEngine.getFacingPosition(playerId);
  ...
  createPlayerAnimations() {
    ...
    console.warn('⚠️ Failed to create player animations:', error);
  } 
  ...
  playWaterAnimation(id, direction) {
    ...
    if (player && player.sprite.anims) {
      try {
        ...
        console.warn('⚠️ Failed to play water animation:', error)
      }
    }
  }
  ...
  playMoveAnimation(id, direction) {
    ...
    if (player && player.sprite.anims) {
      try {
        ...
        console.warn('⚠️ Failed to play move animation:', error)
      }
    }
  }
